...
ccf_res <- ccf(ccf_data$Rain_mm_sum, ccf_data$Secchi_m,
               lag.max = 30, plot = TRUE)

secchi_fut_combined <- fcre_smoothed

# 1. keep only the future rows that aren't already in the big table
new_rows <- noon_future_regs[1:30,] %>%
  anti_join(secchi_fut_combined, by = "datetime")

# 2. append them; bind_rows() fills any extra columns with NA
secchi_fut_combined <- bind_rows(secchi_fut_combined, new_rows)

# tidy result
tail(secchi_fut_combined)


# Forecast lag

secchi_data <- secchi_fut_combined %>%
