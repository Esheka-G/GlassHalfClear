...
secchi_data <- secchi_data %>%
  ...
  drop_na(Secchi_m, Temp_C_mean.x, Bloom_binary_mean.x)  # drop rows with NAs
...
# Multivariate regressor matrix
xreg_all <- as.matrix(secchi_data %>%
                        select(Temp_C_mean.x, Bloom_binary_mean.x))
...
# Make comparison dataframe in long format for legend
multi_forecast_df_long <- multi_forecast_df %>%
  pivot_longer(cols = c(observed, forecast),
               names_to = "type",
               values_to = "Secchi")

# Plot forecast vs observed with legend
ggplot(multi_forecast_df_long, aes(x = time, y = Secchi, color = type)) +
  ...
  labs(
    title = "Multivariate STL + ARIMA Forecast (Temp + Bloom)",
    y = "Secchi Depth",
    x = "Time (Days)"
  ) +
  theme_minimal()
...
cat("Multivariate STL + ARIMA with Temp_C_mean and Bloom_binary_mean\n")
