...
# --- Step 1: Join and create lagged + unlagged predictors ---
...
# --- Step 2: Filter to Nov–Feb (to allow context around Dec–Jan) ---
secchi_data <- secchi_data %>%
  filter(datetime >= as.Date("2023-11-01") & datetime <= as.Date("2024-01-31"))
...
ggplot(forecast_df, aes(x = date)) +
  ...
  labs(
    ...
    subtitle = "Forecast from Late November into December–January",
...
metrics <- data.frame(
  Model = "Chla_lag2 + AirTemp (Dec–Jan)",
